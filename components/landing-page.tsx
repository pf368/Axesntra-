...
const Nav = () => {
  ...
  return (
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '0 40px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: scrolled ? 'rgba(6,7,10,0.95)' : 'transparent', backdropFilter: scrolled ? 'blur(16px)' : 'none', borderBottom: scrolled ? '1px solid var(--border)' : '1px solid transparent', transition: 'all 0.2s ease' }}>
...
const Hero = () => (
  <section style={{ position: 'relative', overflow: 'hidden', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
    <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'var(--bg)' }} />
    <div className="lp-container" style={{ position: 'relative', zIndex: 2, paddingTop: 136, paddingBottom: 80, flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center', minWidth: 0 }}>
        <div style={{ textAlign: 'left' }}>
          <h1 style={{ fontSize: 'clamp(40px, 4.6vw, 64px)', fontWeight: 700, letterSpacing: '-0.035em', lineHeight: 1.04, marginBottom: 22, color: 'var(--text)' }}>
            DOT violations happen.<br />
            <span style={{ color: 'var(--blue-bright)' }}>Axesntra tells you exactly what to do next.</span>
          </h1>
          <p style={{ fontSize: 17, color: 'var(--text-muted)', marginBottom: 36, lineHeight: 1.65, maxWidth: 520 }}>
      ...
      <div style={{ marginTop: 64, display: 'flex', gap: 0, borderTop: '1px solid var(--border)', paddingTop: 28 }}>
        {[{ val: 'FMCSA', sub: 'Official data source' }, { val: 'CSA', sub: 'All 7 BASICs' }, { val: 'SAFER', sub: 'Authority & registration' }, { val: 'Real-time', sub: 'Inspection & violation data' }, { val: 'Plain English', sub: 'AI-generated answers' }].map((s, i) => (
          <div key={i} style={{ flex: 1, paddingRight: 24, borderRight: i < 4 ? '1px solid var(--border)' : 'none', paddingLeft: i > 0 ? 24 : 0 }}>
            <div style={{ fontFamily: 'var(--lp-mono)', fontSize: 13, fontWeight: 600, color: 'var(--blue)', marginBottom: 3 }}>{s.val}</div>
...
export function LandingPage() {
  return (
    <div className="landing-page">
      {/* Grid overlay */}
      <div style={{ position: 'fixed', inset: 0, backgroundImage: 'linear-gradient(rgba(255,255,255,0.022) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.022) 1px, transparent 1px)', backgroundSize: '60px 60px', pointerEvents: 'none', zIndex: 0 }} />
